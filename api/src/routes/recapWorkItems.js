...
export function createRecapWorkItemsRouter(service = recapWorkItemService, publicationService = recapPublicationService) {
    ...
    router.post("/:id/resume-returned-work", async (req, res) => { try { return res.json({ workItem: await service.resumeReturnedWork(req.params.id, req.user, req.body?.expectedVersion) }); } catch (error) { return failure(res, error); } });
