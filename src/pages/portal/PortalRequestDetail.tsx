...
const STATUS_PROGRESS: Record<string, { step: number; label: string; icon: string }> = {
    "Intake Review": { step: 1, label: "Intake Review", icon: "\uD83D\uDD0D" },
    "Work Queue": { step: 2, label: "Work Queue", icon: "\uD83D\uDCCB" },
    "In Progress": { step: 3, label: "In Progress", icon: "\u2699\uFE0F" },
    "Quality Review": { step: 4, label: "Quality Review", icon: "\u2705" },
    Published: { step: 5, label: "Published", icon: "\uD83D\uDCE2" },
};
                ...
                return (
                    <div key={s.step} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                        <div style={{
                            ...
                            background: isActive ? (isCurrent ? "#6366f1" : "#a5b4fc") : "#e2e8f0",
    ...
    return (
        <div className="portal-overview" style={{ maxWidth: 740 }}>
            <div style={{ marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <span style={{ fontSize: 12, color: "#64748b" }}>Request ID: <span style={{ fontFamily: '"SF Mono", "Cascadia Code", "Consolas", monospace', fontWeight: 600, color: "#1e293b" }}>{req.requestId}</span></span>
            </div>

            <div style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 28, background: "#fff", marginBottom: 20, boxShadow: "0 4px 16px rgba(15, 23, 42, 0.06)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
                    <div>
                        <h1 className="po-welcome-title" style={{ fontSize: 22, marginBottom: 4 }}>{req.title}</h1>
                        <span style={{ fontSize: 12, color: "#64748b", display: "block" }}>{req.transactionName} &middot; {req.communityNames[0] || "\u2014"}</span>
                    </div>
                    <StatusBadge status={req.status} />
                </div>

                {STATUS_PROGRESS[req.status] && (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <div style={{ padding: 12, background: "#f8fafc", borderRadius: 8 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.03em", display: "block", marginBottom: 4 }}>Category</span>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "#1e293b" }}>{req.category || "\u2014"}</span>
                    </div>
                    <div style={{ padding: 12, background: "#f8fafc", borderRadius: 8 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.03em", display: "block", marginBottom: 4 }}>Priority</span>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "#1e293b" }}>{req.priority || "\u2014"}</span>
                    </div>
                    <div style={{ padding: 12, background: "#f8fafc", borderRadius: 8 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.03em", display: "block", marginBottom: 4 }}>Last Updated</span>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "#1e293b" }}>{req.updatedAt || req.neededBy || "\u2014"}</span>
                    </div>
                    <div style={{ padding: 12, background: "#f8fafc", borderRadius: 8 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.03em", display: "block", marginBottom: 4 }}>Submitted</span>
            ...
            {!req._publishedExternal && (
                <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 20, background: "#f8fafc", marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#eff6ff", color: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                <circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" />
                            </svg>
                        </div>
                        <div>
                            <span style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", display: "block" }}>Status: {req.status}</span>
                            <span style={{ fontSize: 13, color: "#475569", lineHeight: 1.5, marginTop: 2 }}>
                                {req.status === "Intake Review" && "Your submission is being reviewed by the IntegraCare team to validate the request details."}
                                {req.status === "Work Queue" && "This request is queued and will be assigned to a reviewer."}
                                {req.status === "In Progress" && "This request is actively being worked on by the IntegraCare team."}
                                {req.status === "Quality Review" && "Work is complete and undergoing final review before publication."}
                                {req.status === "Action Needed" && "Additional information is needed from your side. Check for open clarifications."}
            ...
            {req._publishedExternal && (
                <div style={{ border: "1px solid #bbf7d0", borderRadius: 12, padding: 24, background: "linear-gradient(135deg, #f0fdf4 0%, #f8faff 100%)", marginBottom: 20 }}>
                        ...
                        <div>
                            ...
                            {publishedDocs.slice(0, 20).map((doc) => (
                                <div key={doc.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #dcfce7" }}>
                                    ...
                                    <span style={{ fontSize: 11, color: "#94a3b8", fontStyle: "italic" }}>Available</span>
                                </div>
                            ))}
                        </div>
            ...
            {publishedArtifacts.length > 0 && (
                <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 20, background: "#fff", marginBottom: 20, boxShadow: "0 2px 8px rgba(15, 23, 42, 0.04)" }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                        ...
                        Supporting Artifacts ({publishedArtifacts.length})
                    </span>
                    {publishedArtifacts.map((art) => (
                        <div key={art.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f1f5f9" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: 20, background: "#fff", marginBottom: 20, boxShadow: "0 2px 8px rgba(15, 23, 42, 0.04)" }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                ...
                {externalMessages.length > 0 ? (
                    <div>
                        {externalMessages.map((msg) => (
                            <div key={msg.id} style={{ padding: "12px 0", borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <div style={{ textAlign: "center", padding: "20px 0", border: "1px dashed #e2e8f0", borderRadius: 8, background: "#fafbfc" }}>
