...
export default function DocumentHubFind({ refreshKey = 0, canEditMetadata: canEditOverride }: { refreshKey?: number; canEditMetadata?: boolean }) {
    ...
    function beginEdit() {
        if (!selected) return;
        setEditValues({ documentTitle: selected.documentTitle || null, documentTypeKey: selected.documentType?.key || null,
    ...
    async function saveEdit() {
        ...
        try {
            ...
            setSelected(updated); setArtifacts(rows => rows.map(row => row.id === updated.id ? updated : row)); setEditValues(null);
        } catch (cause) { setDownloadError(cause instanceof ArtifactUploadError ? cause.message : "Document details could not be saved."); }
    }
    ...
    return <section className="document-hub-find-workspace" aria-labelledby="find-title">
        ...
        {!loading && !error && artifacts.length > 0 && <>
            <div className="document-hub-results" role="list">
                {artifacts.map(artifact => <button type="button" role="listitem" key={artifact.id} className={selected?.id === artifact.id ? "selected" : ""} onClick={() => { setSelected(artifact); setDownloadError(null); setEditValues(null); }}>
        ...
        {selected && <aside className={`document-hub-detail${canEditMetadata ? "" : " metadata-read-only"}`} aria-label="Document details" aria-modal="true" role="dialog">
            <div className="document-hub-detail-heading"><div><small>Document details</small><h3>{selected.documentTitle || selected.fileName}</h3></div><button type="button" aria-label="Close document details" onClick={() => { setSelected(null); setEditValues(null); }}>×</button></div>
            <section className="document-hub-detail-section"><h4>Document</h4><dl><div><dt>Title</dt><dd>{selected.documentTitle || selected.fileName}</dd></div><div><dt>Original filename</dt><dd>{selected.fileName}</dd></div><div><dt>Document type</dt><dd>{selected.documentType?.displayName || "Not provided"}</dd></div><div><dt>Business topic</dt><dd>{selected.businessTopic?.name || "Not provided"}</dd></div>{selected.description && <div><dt>Description</dt><dd>{selected.description}</dd></div>}<div><dt>Document origin</dt><dd>{selected.documentOrigin || "Not provided"}</dd></div></dl></section><section className="document-hub-detail-section"><h4>Availability</h4><dl><div><dt>Destination</dt><dd>{availabilityLabel(selected)}</dd></div></dl></section><section className="document-hub-detail-section"><h4>Provenance</h4><dl><div><dt>Uploaded by</dt><dd>{selected.submittedByDisplayName || "Unknown user"}</dd></div><div><dt>Uploaded</dt><dd>{formatDate(selected.uploadedAt)}</dd></div>{selected.effectiveDate && <div><dt>Effective date</dt><dd>{formatDate(selected.effectiveDate)}</dd></div>}<div><dt>File type</dt><dd>{friendlyType(selected.extension)}</dd></div><div><dt>Size</dt><dd>{formatDocumentSize(selected.size)}</dd></div></dl></section>
            {!editValues ? <button type="button" className="document-hub-edit-details" onClick={beginEdit}>Edit details</button> : <div className="document-hub-edit-form"><label>Document title<input value={editValues.documentTitle || ""} maxLength={255} onChange={event => setEditValues({ ...editValues, documentTitle: event.target.value || null })} /></label><label>Document type<select value={editValues.documentTypeKey || ""} onChange={event => setEditValues({ ...editValues, documentTypeKey: event.target.value || null })}><option value="">Not provided</option>{metadataOptions.documentTypes.map(type => <option key={type.key} value={type.key}>{type.displayName}</option>)}</select></label><label>Business topic<select value={editValues.businessTopicSlug || ""} onChange={event => setEditValues({ ...editValues, businessTopicSlug: event.target.value || null })}><option value="">Not provided</option>{metadataOptions.businessTopics.map(topic => <option key={topic.slug} value={topic.slug}>{topic.name}</option>)}</select></label><label>Document origin<input value={editValues.documentOrigin || ""} maxLength={255} onChange={event => setEditValues({ ...editValues, documentOrigin: event.target.value || null })} /></label><label>Description<textarea rows={4} maxLength={2000} value={editValues.description || ""} onChange={event => setEditValues({ ...editValues, description: event.target.value || null })} /></label><div><button type="button" onClick={() => setEditValues(null)}>Cancel</button><button type="button" className="document-hub-primary" onClick={saveEdit}>Save details</button></div></div>}
