...
export default function AuthoritativeRecapWorkspace({ initialItem }: { initialItem: RecapRequest }) {
    ...
    return <div className="rc-page" style={{ maxWidth: 1120 }} data-testid="authoritative-recap-workspace">
        ...
        {(actionButtons.length > 0 || capabilities.canUploadArtifact) && <section className="rc-card rc-action-center" style={{ padding: 22 }} data-testid="authoritative-action-center"><div style={{ marginBottom: 16 }}><h2 style={{ margin: 0 }}>Action Center</h2><div style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>{item.status === "Assigned" ? "Accept or return the assignment before beginning work." : item.status === "In Progress" ? "Continue the response, supporting documents, or workflow." : "Actions available for the current authoritative state."}</div></div>
            {actionButtons.some(action => action.primary) && <div className="rc-action-group"><div className="rc-action-group-label">Primary workflow</div><div className="rc-action-grid">{actionButtons.filter(action => action.primary).map(action => <button key={action.label} className="rc-action-tile rc-action-tile-primary" disabled={busy} onClick={action.action}><strong>{action.label}</strong><span>{action.description}</span></button>)}</div></div>}
            {capabilities.canUploadArtifact && <div className="rc-action-group"><div className="rc-action-group-label">Work action</div><div className="rc-supporting-upload">
            ...
            {actionButtons.some(action => !action.primary) && <div className="rc-action-group"><div className="rc-action-group-label">Exception and secondary actions</div><div className="rc-action-grid rc-action-grid-secondary">{actionButtons.filter(action => !action.primary).map(action => <button key={action.label} className="rc-action-tile" disabled={busy} onClick={action.action}><strong>{action.label}</strong><span>{action.description}</span></button>)}</div></div>}
        </section>}
        ...
        {(capabilities.canViewArtifacts || capabilities.canUploadArtifact) && <section className="rc-card rc-document-library" style={{ padding: 20 }}><h2>Documents</h2><div className="rc-document-group"><h3>Original request document</h3>{sources.length === 0 ? <p>No source document is available.</p> : sources.map(source => <div className="rc-document-row" key={source.id}><div className="rc-document-name"><span aria-hidden="true">▤</span><strong><span>Source:</span> <span>{source.fileName}</span></strong></div><button className="rc-btn rc-btn-secondary rc-btn-sm" aria-label={`Download source ${source.fileName}`} onClick={() => void downloadAuthoritativeSourceDocument(item.id, source.id, source.fileName)}>Download source</button></div>)}</div><div className="rc-document-group"><h3>Supporting documents</h3>{artifacts.map(artifact => <div className="rc-document-row" key={artifact.id}><div className="rc-document-name"><span aria-hidden="true">▤</span><div><strong>{artifact.fileName}</strong>{artifact.uploadedAt && <span>Uploaded {dateTime(artifact.uploadedAt)}</span>}</div></div><button className="rc-btn rc-btn-secondary rc-btn-sm" aria-label={`Download ${artifact.fileName}`} onClick={() => void downloadAuthoritativeArtifact(item.id, artifact.id, artifact.fileName)}>Download</button></div>)}{artifacts.length === 0 && <p>No supporting documents uploaded yet.</p>}</div></section>}
