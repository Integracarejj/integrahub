...
export default function AuthoritativeRecapWorkspace({ initialItem }: { initialItem: RecapRequest }) {
    ...
    return <div className="rc-page" style={{ maxWidth: 1120 }} data-testid="authoritative-recap-workspace">
        ...
        {!item.assignedUserId && <section className="rc-card" style={{ padding: 20, borderLeft: "4px solid #2563eb" }}>
            <h2 style={{ marginTop: 0 }}>Assignment required</h2>
            <p>This request is queued and cannot be worked until DD Operations assigns an owner.</p>
            {capabilities.canAssign ? <label style={{ display: "flex", alignItems: "center", gap: 10, fontWeight: 600 }}>Assign Owner<select aria-label="Assign Owner" defaultValue="" disabled={busy} onChange={event => { const selected = assignees.find(candidate => candidate.id === event.target.value); if (selected) setAssignment({ userId: selected.id, displayName: selected.displayName || selected.email || "selected owner" }); event.currentTarget.value = ""; }}><option value="" disabled>Select an owner</option>{assignees.map(candidate => <option key={candidate.id} value={candidate.id}>{candidate.displayName || candidate.email}</option>)}</select></label> : <button className="rc-btn rc-btn-ghost" onClick={() => navigate("/recapitalization/dd-operations")}>Back to Work Queue</button>}
        </section>}
        ...
        {item.assignedUserId && <section className="rc-card" style={{ padding: 20 }}><h2>DD response</h2><textarea aria-label="DD response" rows={10} value={response} disabled={!capabilities.canUpdateResponse} onChange={event => setResponse(event.target.value)} style={{ width: "100%" }} />{response.length > 100000 && <div role="alert" style={{ color: "#b91c1c" }}>Response must be 100,000 characters or fewer. Nothing has been truncated or saved.</div>}<div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><span style={{ color: "#64748b", fontSize: 12 }}>{response.length.toLocaleString()} / 100,000 characters</span>{capabilities.canUpdateResponse && <button className="rc-btn rc-btn-primary" disabled={busy || !response.trim() || response.length > 100000} onClick={() => void run(() => updateAuthoritativeResponse(item.id, response), "Response saved.", false)}>Save response</button>}</div>{item.authoritativeResponse && <div style={{ color: "#64748b", fontSize: 12, marginTop: 8 }}>Saved in the authoritative workspace.</div>}</section>}
